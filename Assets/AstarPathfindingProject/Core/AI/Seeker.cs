...
public class Seeker : MonoBehaviour {
	 ...
	 * \a callback will be called when the path has completed.
	 * \a Callback will not be called if the path is canceled (e.g when a new path is requested before the previous one has completed) */
	public Path StartPath (Path p, OnPathDelegate callback){return StartPath (p, callback, -1);}
	public Path StartPath (Path p, int graphMask){return StartPath (p, null, graphMask);}
	public Path StartPath (Path p){return StartPath (p, null, -1);}
